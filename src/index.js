...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  if (isPro || todosAmount < 10) return next();

  return response.status(403).json({ error: "Create ToDo not available" });
}
...
function checksTodoExists(request, response, next) {
  ...
  if (!isUUID) return response.status(400).json({ error: "Not a valid ID" });
